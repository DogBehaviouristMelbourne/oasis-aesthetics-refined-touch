...
export const BookingSystem = () => {
	...
	const handleBooking = (e: React.FormEvent) => {
		...
		toast({
			...
			description: `We'll contact you soon to confirm your ${selectedService} appointment on ${format(
				selectedDate,
				'PPP'
			)} at ${selectedTime}.`,
		});
		...
		<section className="py-16 px-4 lg:px-6 bg-oasis-contrast">
			<div className="container mx-auto max-w-4xl">
				...
				<Card className="border-oasis-taupe/20">
					<CardHeader>
						<CardTitle className="text-lg font-playfair text-oasis-espresso">
							Schedule Your Visit
						</CardTitle>
					</CardHeader>
					<CardContent>
						<form onSubmit={handleBooking} className="space-y-6">
							{/* Service Selection */}
							<div>
								<Label className="text-sm font-medium text-oasis-espresso mb-3 block">
									Select Service
								</Label>
								<div className="grid md:grid-cols-2 gap-3">
									{services.map((service) => (
										...
										>
											<div className="font-medium text-sm text-oasis-espresso">
												{service.name}
											</div>
							...
							<div className="grid md:grid-cols-2 gap-4">
								<div>
									<Label
										htmlFor="name"
										className="text-sm font-medium text-oasis-espresso"
									>
										Name *
									</Label>
									<Input
				...
				<div className="mt-8 p-4 bg-oasis-contrast rounded-xl shadow text-oasis-espresso text-sm">
					<strong>Cancellation policy:</strong> You can cancel or reschedule up to 4
